  ...
  class Recipe
    ...
    def generate(template : String, options = nil)
      ...
      when "app"
        if options
          ...
          if options.deps?
            ...
            CLI::ProcessRunner.run("cd #{name} && shards update").wait
          end
        end
      when "controller"
      ...
      when "model"
        ...
        if model == "crecto"
          CLI::CrectoMigration.new(name, @fields).render(directory, list: true, color: true)
        else
          CLI::GraniteMigration.new(name, @fields).render(directory, list: true, color: true)
        end
...
class Teeplate::RenderingEntry
  ...
  def list(s, color)
    CLI.logger.info s.colorize.fore(color).to_s + local_path, "Generate", :light_cyan
  end
end
