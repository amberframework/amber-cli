  ...
  class Recipe
    ...
    def generate(template : String, options = nil)
      ...
      when "model"
        ...
        if model == "crecto"
          AmberCLI::CrectoMigration.new(name, @fields).render(directory, list: true, color: true)
        else
          AmberCLI::GraniteMigration.new(name, @fields).render(directory, list: true, color: true)
        end
    ...
    def info(msg)
      AmberCLI.logger.info msg, "Generate", :light_cyan
    end

    def error(msg)
      AmberCLI.logger.error msg, "Generate", :light_red
    end
  end
end
...
class Teeplate::RenderingEntry
  ...
  def list(s, color)
    AmberCLI.logger.info s.colorize.fore(color).to_s + local_path, "Generate", :light_cyan
  end
end
