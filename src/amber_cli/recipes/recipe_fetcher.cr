    ...
    def fetch_zip(url : String, template_path : String)
      ...
      HTTP::Client.get(url) do |response|
        if response.status_code != 200
          AmberCLI.logger.error "Could not find the recipe #{@name} : #{response.status_code} #{response.status_message}", "Generate", :light_red
          return nil
        end
      ...
      AmberCLI.logger.error "Cannot generate #{@kind} from #{@name} recipe", "Generate", :light_red
      return nil
    end
